...
import { Clock, CheckCircle, Package, XCircle, Truck, AlertCircle } from 'lucide-react';
...
const ShopOrdersList = ({ onClose }) => {
  ...
  }, [selectedShop?.id_shop]); // update: Only depend on shop ID, not the entire fetchShopOrders function
  ...
  const calculateOrderTotal = (order) => {
    ...
    if (order.order_products && Array.isArray(order.order_products)) {
      order.order_products.forEach(item => {
        //update: Ensure we're working with numbers
    ...
    //update: If order has a total_price property, use it as fallback
